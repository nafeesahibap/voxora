...
export default {
    ...
    view: () => {
        ...
        const renderTaskItem = (task) => {
            ...
            const isVoice = task.voiceCreated;
                ...
                <div class="task-item" style="display: flex; align-items: center; gap: 1rem; padding: 1rem; background: rgba(255,255,255,0.02); border: 1px solid var(--border-subtle); border-radius: 8px; transition: all 0.2s;">
                    ...
                    <div style="flex: 1; opacity: ${isDone ? 0.5 : 1};">
                        ...
                        <div style="display: flex; gap: 0.5rem; margin-top: 0.25rem; flex-wrap: wrap;">
                            ...
                            ${task.dueDate ? `<span style="font-size: 0.75rem; color: var(--text-muted);"><i class="ph ph-calendar"></i> ${task.dueDate}</span>` : ''}
                        </div>
                    </div>
    ...
    afterRender: () => {
        ...
        document.getElementById('btn-voice-task')?.addEventListener('click', () => {
            ...
            btn.innerHTML = '<i class="ph-bold ph-spinner" style="animation: spin 1s linear infinite"></i> Listening...';
            ...
            setTimeout(() => {
                const voiceTask = {
                    id: Date.now(),
                    title: "Follow up with candidate about offer status",
                    priority: "medium",
                    status: "pending",
                ...
                Store.addTask(voiceTask);
                showToast('🎤 Voice task created: "' + voiceTask.title + '"', 'success');
                window.router.handleRoute();
            }, 2000);
        });
