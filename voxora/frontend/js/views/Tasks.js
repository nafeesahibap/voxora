...
export default {
    ...
    view: async () => {
        // Fetch only if tasks list is currently empty
        ...
        const renderTaskItem = (task) => {
            ...
            const isVoice = task.voice_created === true || task.voice_created === 'true';
        ...
        const totalCount = Store.state.tasks.length;
                ...
                <div class="card card-g-4" style="padding: 1rem 1.5rem;">
                    <div style="display: flex; align-items: center; gap: 1rem;">
                        ...
                        <div>
                            ...
                            <div id="tasks-count-pending" style="font-size: 1.5rem; font-weight: 700;">${pendingCount}</div>
                        </div>
                    </div>
                </div>
                <div class="card card-g-4" style="padding: 1rem 1.5rem;">
                    <div style="display: flex; align-items: center; gap: 1rem;">
                        ...
                        <div>
                            ...
                            <div id="tasks-count-completed" style="font-size: 1.5rem; font-weight: 700;">${completedCount}</div>
                        </div>
                    </div>
                </div>
                <div class="card card-g-4" style="padding: 1rem 1.5rem;">
                    <div style="display: flex; align-items: center; gap: 1rem;">
                        ...
                        <div>
                            ...
                            <div id="tasks-count-total" style="font-size: 1.5rem; font-weight: 700;">${totalCount}</div>
                        </div>
                    </div>
                </div>
            </div>
    ...
    afterRender: () => {
        const refreshTasksUI = () => {
            ...
            const completedEl = document.getElementById('tasks-count-completed');
            if (completedEl) completedEl.textContent = String(completed);

            const totalEl = document.getElementById('tasks-count-total');
            if (totalEl) totalEl.textContent = String(total);

            document.querySelectorAll('.filter-category').forEach(btn => {
                const isActive = btn.dataset.cat === activeCategory;
                btn.style.background = isActive ? 'var(--accent-primary)' : 'rgba(255,255,255,0.05)';
            ...
            taskListEl.innerHTML = filteredTasks.length > 0
                ? filteredTasks.map(renderTaskItem).join('')
                : '<div style="text-align: center; color: var(--text-muted); padding: 2rem;"><i class="ph ph-check-circle" style="font-size: 2rem; opacity: 0.3;"></i><p>No tasks in this category</p></div>';
        };

        window.VoxoraTasksUI = window.VoxoraTasksUI || {};
        window.VoxoraTasksUI.refresh = refreshTasksUI;

        if (!Store._tasksDomSyncSubscribed) {
            Store.subscribe(() => {
                if (document.getElementById('tasks-list')) refreshTasksUI();
            });
            Store._tasksDomSyncSubscribed = true;
        }
        ...
        // Real Voice Recognition Integration - Using Global Voice Engine
        document.getElementById('btn-voice-task')?.addEventListener('click', () => {
            if (window.voiceEngine) {
                window.voiceEngine.start();
            } else {
                showToast("Voice Recognition system not ready.", "error");
            }
        });
